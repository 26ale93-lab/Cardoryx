...
from scripts.test_variant_finish_audit import VERIFIED_XY_SM_CHECKLIST_FINISHES
...
normal_ids={cid.lower() for cid,row in VERIFIED_XY_SM_CHECKLIST_FINISHES.items() if "Normal" in row["finishes"]}
holo_ids={cid.lower() for cid,row in VERIFIED_XY_SM_CHECKLIST_FINISHES.items() if "Holo" in row["finishes"]}
reverse_ids={cid.lower() for cid,row in VERIFIED_XY_SM_CHECKLIST_FINISHES.items() if "Reverse Holo" in row["finishes"]}

assert len(VERIFIED_XY_SM_CHECKLIST_FINISHES)==42
assert len(normal_ids)==34
assert len(holo_ids)==8
assert len(reverse_ids)==37
...
rows={cid.lower():{"setId":r["setId"],"localId":r["localId"]} for cid,r in VERIFIED_XY_SM_CHECKLIST_FINISHES.items()}
js=f"""
...
for(const id of Object.keys(rows)){{
  ...
  if(verifiedNormalFinish(wrongSet)||verifiedHoloFinish(wrongSet)||verifiedReverseFinish(wrongSet))throw new Error('wrong set '+id);
  if(verifiedNormalFinish(wrongLocal)||verifiedHoloFinish(wrongLocal)||verifiedReverseFinish(wrongLocal))throw new Error('wrong local '+id);
}}
const fake={{id:'xy1-999',tcgdexId:'xy1-999',localId:'999',set:{{id:'xy1'}}}};
