...
old = """function verifiedSpecialStampFinishes(card,stamp){
...
"""
new = """function verifiedSpecialStampFinishes(card,stamp){
  const id=String(card?.tcgdexId||card?.id||'').trim().toLowerCase();
  const rule=VERIFIED_SPECIAL_STAMP_FINISHES[id];
  if(!rule || cardSetId(card)!==rule.setId ||
     exactLocalIdKey(card?.localId||'')!==exactLocalIdKey(rule.localId) ||
...
// No marketplace product or price is stored here: this only unlocks a physical
// finish for one exact tcgdexId/set/localId/Series identity.
...
// V2.1.14 — Prize Pack Series 1–9 resolver + exact product refresh.
"""
index = replace_once(index, old, new, "insert exact Play finish registry")

old = """  if(stamp==='Play! Pokémon'){
...
"""
new = """  if(stamp==='Play! Pokémon'){
    const plan=prizePackFinishPlan(card,series);
    if(plan.authoritative && plan.finishes.length)allowed=new Set(plan.finishes.map(canonicalVariant));
    // Exact finish evidence affects availability only. It is intentionally kept
    // outside prizePackFinishPlan(), so it cannot create a product or price.
  ...
  if(!allowed.size)allowed.add('Non so');
"""
index = replace_once(index, old, new, "apply exact Play finish evidence")
...
# Focused production guard: use the real registry/function from index.html and
# execute them with Node. Build the JS source line-by-line to avoid nested quote
# fragility in this temporary patch helper.
helper_lines = [
    "",
    ...
    "    names = (\"normText\", \"canonicalVariant\", \"exactLocalIdKey\", \"cardSetId\", \"normalizedPlaySeries\", \"verifiedPlaySeriesFinishes\")",
...
test = replace_once(test, marker, helper_code + "def main():\n    ap = argparse.ArgumentParser()\n", "insert Quilava runtime test")
...
insert = """    if args.quilava_play_only:
    ...
    if args.numel_ditto_only:
        real_world_regression = real_world_regression_audit(registries, play_index, set(), args.workers)
"""
test = replace_once(test, focused, insert, "add Quilava focused test mode")
