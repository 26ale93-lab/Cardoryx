...
assert "Le quotazioni non più disponibili non vengono conteggiate come perdita." in source
...
const before=new Map([
 ['up',{{key:'up',name:'A',qty:2,value:1}}],
 ['down',{{key:'down',name:'B',qty:3,value:2}}],
 ['new',{{key:'new',name:'C',qty:4,value:null}}],
 ['lost',{{key:'lost',name:'D',qty:5,value:0.3}}],
 ['same',{{key:'same',name:'E',qty:1,value:1}}],
]);
const after=new Map([
 ['up',{{key:'up',name:'A',qty:2,value:1.5}}],
 ['down',{{key:'down',name:'B',qty:3,value:1.5}}],
 ['new',{{key:'new',name:'C',qty:4,value:0.4}}],
 ['lost',{{key:'lost',name:'D',qty:5,value:null}}],
 ['same',{{key:'same',name:'E',qty:1,value:1}}],
]);
const r=buildPriceRefreshReport(before,after);
...
assert.strictEqual(by.up.delta,0.5);
assert.strictEqual(by.up.percent,50);
assert.strictEqual(by.up.totalDelta,1);
assert.strictEqual(by.down.kind,'down');
assert.strictEqual(by.down.delta,-0.5);
assert.strictEqual(by.down.percent,-25);
assert.strictEqual(by.down.totalDelta,-1.5);
assert.strictEqual(by.new.kind,'new');
assert.strictEqual(by.new.totalDelta,1.6);
assert.strictEqual(by.lost.kind,'lost');
assert.strictEqual(by.lost.totalDelta,null);
assert(Math.abs(r.net-1.1)<1e-9,r.net);
assert(!by.same);
console.log(JSON.stringify({{
  changes:r.changes.length,up:r.up,down:r.down,newCount:r.newCount,lost:r.lost,net:r.net,
  languageSeparated:true,lostNotCountedAsLoss:true
}}));
"""
